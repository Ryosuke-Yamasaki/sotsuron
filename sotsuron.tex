...
\clearpage
\subsection{使用例}
本研究で開発したシミュレータ教材は常微分方程式の基礎的な知識を持っている前提で演習問題を解く際の補助教材として使用することを想定している．図\ref{fig:シミュレータ教材を開いた際の画面}が開発した教材を開いた際の画面である．図\ref{fig:シミュレータ教材を開いた際の画面}から導関数や微小区間の幅を設定してシミュレーションをすることができる．
...
まず，本教材を用いた演習の例について記述していく．導関数に$\sin(x)$，微小区間の幅を0.1，必要な精度を0.01を設定してStartボタンを押した際の画面を図\ref{fig:シミュレータ教材の使用例}に示す．
...
\clearpage
\subsection{実装機能}
画面右上に，オイラー法によって計算された原関数のグラフを赤色のラインで，理論値のグラフを緑色のラインで表されている．画面右下に，オイラー法によって計算された数値の中で理論値との差が最大となる$x$付近を拡大されたグラフをオイラー法によって計算された原関数のグラフを赤色のラインで，理論値のグラフを緑色のラインで表されている．%　とりあえずここに避難させてるだけ．ここの節で書きたいもののはず
\subsubsection{操作部分}
...
\subsubsection{結果部分}
結果部分を図\ref{fig:結果部分}に示す．結果部分はシミュレーションをする範囲の最大と微小区間の幅の商を計算コストとし，それを表示する試行回数，オイラー法によって計算された数値のと理論値の差を誤差とし，誤差の大きい順に5つの$x$の値と誤差の表，操作部分で入力された必要な精度を満たしているかの判定文及び再計算を行なった際に前回の計算の誤差の大きい順に5つの$x$の値と誤差の表を表示した．ここでは微小区間の幅を変更することによって誤差がどの程度変化したかを数値で確認することができる．
